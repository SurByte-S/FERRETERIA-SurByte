...
begin
  ...
  select
    item.ordinality::int,
    upper(trim(coalesce(item.value ->> 'codigo', ''))),
    case
  ...
  select
    ...
    inactive_product.match_count as inactive_match_count,
  ...
  left join public.products sku_product
    ...
    and sku_product.active = true
    and upper(trim(sku_product.sku)) = consolidated.code
  left join lateral (
    select
    ...
    where product.tenant_id = input_tenant_id
      ...
      and upper(trim(product.barcode)) = consolidated.code
  ...
  left join lateral (
    ...
    where product.tenant_id = input_tenant_id
      ...
      and (
        upper(trim(product.sku)) = consolidated.code
        or upper(trim(product.barcode)) = consolidated.code
      )
  ) inactive_product on coalesce(sku_product.id, barcode_product.product_id) is null;
  ...
  from stock_csv_matches
  where product_id is null
    and inactive_match_count > 0;

  if inactive_count > 0 then
    raise exception 'STOCK_CSV_PRODUCT_INACTIVE';
  end if;
